...
namespace gspro_r10
{
    ...
    {
        public static TrajectoryResult Calculate(double ballSpeedMph, double launchAngleDeg, double backSpinRpm, double airDensityKgM3 = 1.225)
        {
            ...
            for (int i = 0; i < 12000; i++)
            {
                ...
                if (z < 0 && time > dt)
                {
                    ...
                    double landingSpeedMph = Math.Sqrt(vx * vx + vz * vz) / 0.44704;

                    return new TrajectoryResult(landingX * 1.0936133, apex * 1.0936133, landingTime, landingSpeedMph);
                }
            }

            return new TrajectoryResult(Math.Max(0, x * 1.0936133), Math.Max(0, apex * 1.0936133), time, Math.Sqrt(vx * vx + vz * vz) / 0.44704);
        }
